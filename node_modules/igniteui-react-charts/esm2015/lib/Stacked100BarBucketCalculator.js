...
export let Stacked100BarBucketCalculator = /*@__PURE__*/ (() => {
    class Stacked100BarBucketCalculator extends StackedBarBucketCalculator {
        ...
        getBucket1(a, b, c, d, e, f) {
            let g = typeCast(StackedBarSeries.$, this.c.ct);
            let h = [NaN, NaN, NaN];
            ...
            let r = Math.min(g.te != null ? g.te.length : 0, g.td != null ? g.td.length : 0);
            let s = c * this.d;
            let t = Math.min(s + this.d - 1, r - 1);
            for (let u = s; u <= t; ++u) {
                j = a.valueColumn.item(u);
                k = Math.abs(g.te[u]) + g.td[u];
                if (j < l) {
                    p = Math.min(p, (i.logicalSeriesLink.a0._inner[u] + j) / k * 100);
                    o = Math.max(o, i.logicalSeriesLink.a0._inner[u] / k * 100);
                }
                else {
                    p = Math.min(p, i.logicalSeriesLink.az._inner[u] / k * 100);
                    o = Math.max(o, (i.logicalSeriesLink.az._inner[u] + j) / k * 100);
                }
                if (!isNaN_(m)) {
                    if (!isNaN_(p)) {
                        m = Math.min(m, p);
            ...
            let v = new ScalerParams(0, d, e, g.xAxis.cg, q);
